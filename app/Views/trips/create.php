...
// Vue : création d’un trajet
// Le header/footer + Bootstrap sont gérés par le layout global
?>
...
<!-- Bloc informatif : conducteur (pré-rempli et non modifiable) -->
<div class="card mb-3">
  <div class="card-body">
    <h2 class="h6">Informations conducteur (non modifiables)</h2>

    <?php
      // Sécurisation : on force en string pour éviter les warnings
    ...
    <p class="mb-1">
      <strong>Nom :</strong>
      <?= htmlspecialchars(trim($first . ' ' . $last)) ?>
    </p>
...
<!--
  ...
  - POST obligatoire (action sensible)
  - CSRF obligatoire
-->
<form method="post" action="<?= BASE_URL ?>/trip/create" class="row g-3">
  ...
  <div class="col-md-6">
    ...
    <select class="form-select" name="departure_agency_id" required>
      ...
      <?php foreach ($agencies as $a): ?>
        <option
          value="<?= (int)$a['id'] ?>"
          <?= ((string)($form['departure_agency_id'] ?? '') === (string)$a['id']) ? 'selected' : '' ?>
        >
  ...
  <div class="col-md-6">
    ...
    <select class="form-select" name="arrival_agency_id" required>
      ...
      <?php foreach ($agencies as $a): ?>
        <option
          value="<?= (int)$a['id'] ?>"
          <?= ((string)($form['arrival_agency_id'] ?? '') === (string)$a['id']) ? 'selected' : '' ?>
        >
          <?= htmlspecialchars($a['name']) ?>
        </option>
      <?php endforeach; ?>
  ...
  <div class="col-md-6">
    ...
    <input
      ...
      type="datetime-local"
      name="departure_at"
      value="<?= htmlspecialchars((string)($form['departure_at'] ?? '')) ?>"
      required
    >
  </div>
  ...
  <div class="col-md-6">
    ...
    <input
      ...
      type="datetime-local"
      name="arrival_at"
      value="<?= htmlspecialchars((string)($form['arrival_at'] ?? '')) ?>"
      required
    >
  </div>
  ...
  <div class="col-md-6">
    ...
    <input
      ...
      min="0"
      value="<?= (int)($form['available_seats'] ?? 0) ?>"
      required
    >
